...
class MetricsFetcher:
    ...
    def __init__(self, k8s_api, prom_api, queries, logger, scheduler_interval=100, use_apps=False):
    ...
    def write_to_csv(self, data):
        ...
        with open(self.csv_file, "a", newline="") as f:
            writer = csv.writer(f)
            if f.tell() == 0:
                writer.writerow(["timestamp", "app_name"] + [q["metricName"] for q in self.queries])
            for row in rows:
                writer.writerow(row)
